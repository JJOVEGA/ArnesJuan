...
| [REQ-025](REQ-025.md) | El arnés vigila también a quien orquesta: la sesión coordinadora acredita, decide y publica, y ninguna puerta la mide. **Ventana 1.35.0** (movida el 2026-09-21); línea base = 20 errores de coordinación catalogados. **Entrega 1 acotada**: la sede escrita de la coordinación orientada a entregas (seis reglas del propietario) + gemela en `templates/` y entrada de `arnes-upgrade` **preparadas, no publicadas**. Sigue en `borrador`: **tres conflictos** esperan decisión del propietario | `borrador` | critico | pendiente | pendiente |
