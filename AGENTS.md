...
> límites: `docs/gobernanza/autoalojamiento.md` § «Enmienda: autoalojamiento aligerado». **Nació
> cambiando sólo cómo desarrollamos este repositorio; desde el levantamiento del límite 1
> (2026-09-12 plantillas, 2026-09-13 agentes distribuidos) una parte nombrada SÍ llega a los
> proyectos: la vía proporcional de reparación, en `templates/AGENTS.md.tpl`, en los cuatro agentes
> y en la migración de `arnes-upgrade`. Ninguna otra parte de la enmienda se propaga sin una
> decisión nueva.** Lo que sigue es el régimen del 2026-09-05, vigente en todo lo que la enmienda
> no toca.
>
...
> del plugin no la ejerce por tenerla escrita en su definición, y el texto que describe la vía no
> la autoriza por estar instalado: **instalar la política y aceptarla son dos actos distintos**, y
> el segundo es del propietario del proyecto. **Aquí ese acto existe y tiene fecha:** ArnesJuan
> **autoriza la vía proporcional de reparación** descrita en esta sección, por la **enmienda
> expresa del propietario del 2026-09-10** citada arriba — no por venir escrita en la plantilla.
> **La contradicción con el «todo REQ pasa por analista, desarrollador, QA
> y auditor» de la cita anterior queda resuelta expresamente ahí y aquí**: manda la enmienda; el
> régimen del 2026-09-05 sigue vigente **en todo lo que la enmienda no toca**.
...
1. **Un agente que el plugin no entrega no recibe la comprobación.** Si un proyecto tiene su propia
   copia de una definición de agente en `.claude/agents/`, esa copia lleva el texto que alguien
   escribió ahí y **ninguna instrucción del plugin puede gobernar un archivo que el plugin no
   entrega**. En ese caso la comprobación queda sólo en manos de la coordinadora (§14 A), que sí la
   lee de **este** documento — y si el coordinador tampoco la tiene (§14, límite (b)), no la hace
   nadie.
...
- **CAMBIOS POR HALLAZGO** (un hallazgo de QA o de seguridad obliga a cambiar comportamiento o a
  ...
  proporcional (§6)** —lo hace—. **Si §6 no la declarara, el write-back sería del
  `analista-requerimientos`**, como antes de que la vía existiera: la ausencia de la declaración
  conserva el procedimiento anterior y **nunca deja este write-back sin dueño**.
  **Quien transcribe no decide**: si al
...
| **El CIERRE juzga DOS cosas distintas sobre cada veredicto, y NO las gobierna el mismo eje: qué VALOR lleva el campo, y que el campo llegue a DECLARARSE.** **Por VALOR manda el rigor efectivo, y los dos umbrales NO coinciden:** un `QA:` cuyo valor no sea `aprobado` no deja cerrar en `estandar` ni en `critico`, y **sí deja cerrar en `ligero`**, que no pide veredicto de QA; un `Seguridad:` cuyo valor no sea `aprobado` no deja cerrar **sólo** en `critico`. En esas celdas la llave `campos.ausencia_exige` **no mueve nada**: ahí el campo **está** declarado. **Por AUSENCIA manda otro eje, y cambia con la clave.** Ausencia es una **propiedad** —que el campo no llegue a declararse—, no una lista de vías: comentarlo, borrarlo, no escribirlo nunca o escribir su clave de forma que este lector no la lea como esa clave (`qa: aprobado`) son ejemplos **no exhaustivos**, y el sitio único de las vías conocidas es `docs/seguridad/registro-seguridad.md`. Para `QA:` el eje es **la llave y no el rigor**, así que **con la llave APAGADA —como nace todo proyecto— un REQ SIN línea `QA:` CIERRA en los TRES rigores, también en `critico`**; encendida, se deniega en los tres nombrando el campo. Para `Seguridad:` el eje sigue siendo el **rigor**: su ausencia no para el cierre por debajo de `critico`, idéntico en los dos estados de la llave. Medido celda a celda en `43/E` y en `40/6` (`QA-024-38`). **Y esta fila habla del CIERRE y de nada más:** FIRMAR la seguridad sobre un REQ cuyo `QA:` no llega a declararse **deniega en los dos estados de la llave**, y eso va en la fila del **orden de las firmas**, porque el orden de las fases no es una opción de proyecto | §9 | `guard-completado` | `Edit`/`Write`/`MultiEdit` |
