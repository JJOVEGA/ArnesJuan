# Sección 45 (2 de 2) del banco — 45-migracion-de-la-tabla-heredada-2-la-guia
...
#
# QUÉ MIDE. La parte 1 ejerce la MIGRACIÓN sobre maquetas de proyecto: clasifica, aplica,
...
# inicializado está CONGELADO (`REQ-024 CA-12 (ii)`, `QA-024-38`).
#
...
#
# LAS SEIS ANCLAS ESTÁN DUPLICADAS RESPECTO DE LA PARTE 1, Y VA DICHO PORQUE ES UNA DECISIÓN.
...
# tapa —que nadie comprueba que las dos copias coincidan— lo cierra el caso `45/17`: si las
# anclas de ESTE archivo no están en la guía, sale rojo; y si la parte 1 usara otras, sus
# casos ejecutables saldrían rojos sobre el documento real. Las dos copias sólo pueden
# quedarse calladas a la vez estando las dos bien.
#
...
PISO_AUTONOMO_SECCION=99  # 44 preámbulo con sus dos declaraciones y el titular (líneas 1-44) + 20 maquinaria propia, que el corredor no tiene: el aplanador del apartado, el reconocedor y el juez de fragmento (líneas 57-76) + 35 bloque indivisible mayor (las tres tablas de anclas, líneas 46-55, y el caso 45/17 que las recorre, líneas 78-102 — se declaran juntos porque ese caso no se lee sin ellas) · REQ-014 CA-18 · REGLA para re-derivar los tres términos sin preguntar: «preámbulo» son las líneas hasta el titular inclusive; un «bloque» es un grupo de líneas consecutivas sin blanca en medio; «maquinaria» es la que esta parte define porque el corredor no la tiene, no una copia de otra parte

seccion_nueva "--- 45/2 · la guia de migracion MANDA la sustitucion (REQ-024 CA-12 (ii), QA-024-38) ---"
...
N45B="45/17 la guia declara las SEIS anclas que la parte 1 ejerce"
...
guia45b "45/18 la guia manda sustituir los tres textos en el AGENTS.md ya congelado del proyecto" \
  'TRES TEXTOS DE `AGENTS.md` §13 QUE HAY QUE SUSTITUIR EN EL PROYECTO'
guia45b "45/19 ...y dice por que no basta corregir la plantilla: el proyecto instalado se queda con las promesas falsas" \
  'un proyecto actualiza a 1.34.0 y **se queda con las promesas falsas**'
guia45b "45/20 ...con el mecanismo existente: merge a tres vias, bloque a bloque, sin sobrescribir AGENTS.md entero" \
  'con el merge a tres vías de esta misma skill, BLOQUE A BLOQUE'
guia45b "45/21 ...y prohibe retocar la instantanea de origen para que el diff salga limpio" \
  'no se retoca antes de comparar, ni para que el diff salga más limpio'
guia45b "45/22 ...y ante un conflicto conserva el contenido y lo publica como pendiente de resolver" \
  'el contenido del proyecto **sigue ahí entero**'
guia45b "45/23 ...y con un conflicto abierto NO declara la migracion completada" \
