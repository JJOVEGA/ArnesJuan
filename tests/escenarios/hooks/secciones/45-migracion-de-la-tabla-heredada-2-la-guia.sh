...
N45B="45/35 la guia declara las TRES anclas de INSERCION que dicen donde va un bloque NUEVO"
if [ -z "$FILTRO" ] || printf '%s' "$N45B" | grep -qi -- "$FILTRO"; then
  if [ -z "$APM45B" ]; then
    echo "  FAIL  $N45B  el apartado «Hacia 1.34.0» de ${SK45B##*/} salio VACIO: no hay guia que medir"; FAIL=$((FAIL+1))
  else
...
guia45b "45/36 la guia hace la PREGUNTA PREVIA antes de las dos cuentas: ¿tenia la base este bloque?" \
  '¿tenía la BASE este bloque?'
guia45b "45/37 ...y dice el efecto de omitirla: el proyecto no puede migrar nunca, sin conflicto real que resolver" \
  'el proyecto no puede migrar nunca'
guia45b "45/38 ...y clasifica NUEVO el bloque que no existia en la base, con su sitio" \
  '| 0 / 0 | **no** | `NUEVO` | **añadir** inmediatamente antes del ancla de inserción, y nada más |'
guia45b "45/39 ...y limita NUEVO a anadir: el resto del AGENTS.md del proyecto no se toca" \
  '**Añade y no sustituye:**'
guia45b "45/40 ...y si el proyecto ya tiene algo en esa ancla, NO es NUEVO: vuelve al camino del conflicto" \
