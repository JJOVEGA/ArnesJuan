...
#
# ORDEN. Primero git destructivo (¿esta orden borra trabajo ajeno?), después identidad
# (¿quién edita?), después cierre (¿se puede cerrar?). Si uno deniega, `arnes_deny`
# termina el proceso y los siguientes no corren — lo cual es correcto: la denegación
# ya es final y no hay nada más que juzgar. Si ninguno deniega, se emiten los AVISOS
# acumulados (un veredicto fuera del vocabulario, p. ej.) como `systemMessage`.
set -uo pipefail
DIR="${BASH_SOURCE[0]%/*}"
