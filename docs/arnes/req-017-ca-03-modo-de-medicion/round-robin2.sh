#!/usr/bin/env bash
# Segunda tanda CONTROLADA. SIN quemadores propios: se mide bajo la carga REAL de la
# maquina (hay otra comision corriendo el banco) y se publica `carga` fila a fila.
# Configuraciones intercaladas round-robin: ninguna ve un entorno privilegiado.
SP=/tmp/claude-1000/-home-juan-dev-ArnesJuan/23a8ee5a-8587-4923-8f15-1fc9bf03aaf2/scratchpad
LIB="$SP/her321/hooks/lib.sh"
OUT="$SP/medidas-rr2.tsv"
printf 'ronda\tconfig\tmin1\tmax1\tmin2\tmax2\tcoc_milesimas\tcarga\tus\n' > "$OUT"
campo() { local reg=" $1" c="$2"; reg="${reg##* $c=}"; printf '%s' "${reg%% *}"; }
for i in 1 2 3 4 5; do
  for cfg in bloque:1:3 bloque:3:9 inter:3:9; do
