...
| Una línea de la cabecera que la máquina **no puede medir** no deja cerrar — la propiedad es el **estado**, no el carácter: la línea lleva dentro algo que nadie ve en el diff y que cambia lo que el lector resuelve, así que una persona lee ahí un campo y la máquina no lo lee como ese campo (o el carácter le **fabrica** un delimitador). Se **deniega** citando **esa línea** y lo insertado en forma **imprimible** (`\xNN`), aunque los veredictos estén en verde, y **nunca** se permite por **ausencia** del campo que ese carácter borró. El **retorno de carro** que no termina la línea es una **instancia**, no la definición; también lo son un BOM (`\xef\xbb\xbf`, el que PowerShell añade al redirigir), un espacio de anchura cero, un byte de control C0, un multibyte partido y un **blanco de más o puesto en el sitio de otro** (`Sensible a  seguridad`). La clase **no** se cierra con una lista de caracteres: retirado de la clave lo ajeno al alfabeto de las claves y sus blancos, si lo que queda **es** una clave del lector, alguien insertó algo dentro. Tres fronteras: el CR/LF **final** es transporte (CRLF decide igual que LF), el **cuerpo** del REQ no se restringe y **reabrir** no se bloquea | §9 | `guard-completado` | `Edit`/`Write`/`MultiEdit` |
